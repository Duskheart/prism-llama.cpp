...
//------------------------------------------------------------------------------
// block_q1_0 - 1-bit quantization with group size 32
//------------------------------------------------------------------------------
#define QK1_0 32

typedef struct {
    half d;                    // delta (scale)
    uchar qs[QK1_0 / 8];       // 4 bytes = 32 bits for 32 weights
} block_q1_0;
...
kernel void kernel_convert_block_q1_0(
    ...
    *d = b->d;
    ...
    // Copy 4 bytes of quantized bits
...
//------------------------------------------------------------------------------
// block_q1_0_g128 - 1-bit quantization with group size 128
//------------------------------------------------------------------------------
#define QK1_0_g128 128

typedef struct {
    half d;                        // delta (scale)
    uchar qs[QK1_0_g128 / 8];      // 16 bytes = 128 bits for 128 weights
} block_q1_0_g128;

// Convert block_q1_0_g128 AoS -> SoA (separate scales and quants)
kernel void kernel_convert_block_q1_0_g128(
    global block_q1_0_g128 * src0,
    global uchar * dst_q,
    global half  * dst_d
) {
    global block_q1_0_g128 * b = (global block_q1_0_g128 *) src0 + get_global_id(0);
    global uchar           * q = (global uchar *) dst_q + (QK1_0_g128/8)*get_global_id(0);
    global half            * d = (global half *) dst_d + get_global_id(0);
...
kernel void kernel_restore_block_q1_0_g128(
    global uchar * src_q,
    global half  * src_d,
    global block_q1_0_g128 * dst
) {
    global block_q1_0_g128 * b = (global block_q1_0_g128 *) dst + get_global_id(0);
    global uchar           * q = (global uchar *) src_q + (QK1_0_g128/8)*get_global_id(0);
    global half            * d = (global half *) src_d + get_global_id(0);
