...
// Q1_0_g128 has 128 elements per block, so we process 16 at a time
// il = 0,16,32,48,64,80,96,112 (8 different starting positions for 64 threads / 8 = 8 groups)
inline float block_q1_0_g128_dot_y_flat(global uchar * x, global half * dh, float16 yl, int il) {
    float d = *dh;
...
inline void mul_vec_q1_0_g128_f32_8x_flat(global uchar * src0_q, global half * src0_d, global float * src1, global float * dst, int ne00, int ne01, int ne02, int ne10, int ne12, int ne0, int ne1, int r2, int r3) {
    const ulong nb = ne00 / QK1_0_G128;
    int r0 = get_group_id(0), r1 = get_group_id(1), im = get_group_id(2);
    int first_row = (r0 * N_SIMDGROUP + get_sub_group_id()) * N_DST;
    int i12 = im % ne12, i13 = im / ne12;
    ulong offset0_d = first_row * nb + (i12/r2)*(nb*ne01) + (i13/r3)*(nb*ne01*ne02);
    ulong offset0_q = (first_row * nb + (i12/r2)*(nb*ne01) + (i13/r3)*(nb*ne01*ne02)) * (QK1_0_G128/8);
    ...
    global float * yb = y + ix * QK1_0_G128 + il;
    ...
    for (int ib = ix; ib < nb; ib += N_SIMDWIDTH/8) {
        ...
        sumf.s0 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 0*nb*(QK1_0_G128/8), d + ib + 0*nb, yl, il);
        sumf.s1 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 1*nb*(QK1_0_G128/8), d + ib + 1*nb, yl, il);
        sumf.s2 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 2*nb*(QK1_0_G128/8), d + ib + 2*nb, yl, il);
        sumf.s3 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 3*nb*(QK1_0_G128/8), d + ib + 3*nb, yl, il);
        sumf.s4 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 4*nb*(QK1_0_G128/8), d + ib + 4*nb, yl, il);
        sumf.s5 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 5*nb*(QK1_0_G128/8), d + ib + 5*nb, yl, il);
        sumf.s6 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 6*nb*(QK1_0_G128/8), d + ib + 6*nb, yl, il);
        sumf.s7 += block_q1_0_g128_dot_y_flat(x + ib*(QK1_0_G128/8) + 7*nb*(QK1_0_G128/8), d + ib + 7*nb, yl, il);
...
#endif
kernel void kernel_mul_mat_q1_0_g128_f32_1d_8x_flat(global uchar * src0_q, global half * src0_d, global float * src1, ulong offset1, global float * dst, ulong offsetd, int ne00, int ne01, int ne02, int ne10, int ne12, int ne0, int ne1, int r2, int r3) {
    src1 = (global float*)((global char*)src1 + offset1);
    dst = (global float*)((global char*)dst + offsetd);
    mul_vec_q1_0_g128_f32_8x_flat(src0_q, src0_d, src1, dst, ne00, ne01, ne02, ne10, ne12, ne0, ne1, r2, r3);
}
